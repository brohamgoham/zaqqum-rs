...
// File: contracts/_AlturaVesting.sol

// Altura - Vest contract
...
contract AlturaVesting is Ownable {
  ...
  constructor(address _altura, address _ido, uint _start) public {
		startTime = _start;

		alturaToken = IBEP20(_altura);

		vests.push(Vest("ido",     TOTAL_SUPPLY.mul(330).div(1000),     _ido,      1, 1,  _start.sub(1),  0,  true )); vestIndices["ido"]   = 0;
  }
  ...
  function setAddress(string calldata name, address _address) external onlyOwner {
	...
	function setAlturaAddress(address _address) external onlyOwner {
    require(_address != address(0x0), "invalid address");
		alturaToken = IBEP20(_address);
  }
	...
	function release() public onlyOwner {
		for(uint i = 0 ; i < vests.length; i++) {
			...
			if(block.timestamp <= lockEnd) {
				...
				if(available > 0) {
					if(alturaToken.transfer(v.receiver, available)) {
	...
	function alturaBalance() view public returns(uint256){
		return alturaToken.balanceOf(address(this));
	}
}